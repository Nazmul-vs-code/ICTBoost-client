...
import CLessonFilter from "@/components/c/CLessonFilter";
...
const CLessonsContent = () => {
  ...
  useEffect(() => {
    ...
    router.replace(`/c${queryString ? `?${queryString}` : ""}`, { scroll: false });
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        const result = await GetApiFunction("/lesson/c");
    ...
    <section className="max-w-6xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-base-content/60 max-w-2xl mx-auto">
          Explore our collection of C programming lessons created by expert teachers.
          Learn from video tutorials and Programiz references.
        </p>
      </div>
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="card bg-base-100 shadow-xl border border-sky-500/15 animate-pulse"
              >

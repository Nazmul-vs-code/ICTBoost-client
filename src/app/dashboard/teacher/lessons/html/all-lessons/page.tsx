...
const AllHtmlLessonsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchLessons = async () => {
      try {
        const result = await GetApiFunctionWithParams("/lesson/html", {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-base-content/50 mt-2 text-sm md:text-base">
            View and manage all your created HTML lessons. 🌐
          </p>
        </div>
        <Link
      ...
      {!loading && lessons.length > 0 && (
        <>
              ...
              >
                <div className="card-body p-5">
                  ...
                  <div className="flex flex-col gap-1.5 mt-3 text-xs">
                    ...
                    >
                      <FaGlobe size={12} /> W3Schools Reference
                    </a>
                  </div>
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5">
            <h3 className="text-base font-bold text-primary">💡 Tip</h3>
            <p className="mt-1 text-sm text-base-content/60">
              You have created {lessons.length} HTML lesson{lessons.length > 1 ? "s" : ""}.
...
export default AllHtmlLessonsPage;
